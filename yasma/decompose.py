...
# sys.exit()
def counters_to_bigwig(counter_d, chromosomes, file):
...
def get_kernel_coverage(bam, rgs, params, chrom_depth_c, chromosomes):
	...
	total_genomic_space = sum([c[1] for c in chromosomes])
	...
	for i, read in enumerate(reads):
		...
		depth_c[chrom].update(list(range(pos-half_cov_window, pos+half_cov_window+1)))

		perc_out = perc.get_percent(i)
		if perc_out:
			print(f"    calculating kernel coverage .. {perc_out}%\t {i:,} reads", end='\r', flush=True)


	print()
	perc = percentageClass(1, total_genomic_space)
	print(f"    calculating kernel mean ...... 0%", end='\r', flush=True)

	for chrom, chrom_length in chromosomes:

		kernel = deque([0]*kde_window)
		rolling_average = 0

		for pos in range(chrom_length):

			kernel_pos = pos-half_kde_window


			depth = depth_c[chrom][pos]
			kernel.append(depth)
			rolling_average -= kernel.popleft() / kde_window
			rolling_average += depth / kde_window

			# if sum(kernel) == 0:
			# 	average = 0
			# else:
			# 	average = mean(kernel)
			# if rolling_average > 8:
			# 	kernel_c[chrom][pos-half_kde_window] = rolling_average
			# else:

			if kernel_pos >= 0:
				kernel_c[chrom][kernel_pos] = rolling_average

				perc_out = perc.update()
				if perc_out:
					print(f"    calculating kernel mean ...... {perc_out}%\t {chrom}:{pos:,}           ", end='\r', flush=True)

	print()


	return(depth_c, kernel_c)

def get_coverage(bam, rgs):
	""" Produces coverages for alignment using samtools depth. 
	Returns a dict() of Counter() objects, where the dict keys are chromosomes, counter keys are chromosome positions, and counter values are depth at positions.
	"""


	sys.exit("Module not functional. Needs to be modified to work on all chromosomes.")
	depth_c = Counter()
...
@cli.command(group='Utilities', help_priority=2)
...
# 	default='creep',
# 	type=click.Choice(['creep', 'bool', 'hyst']),
# 	help="Choice between two methods for finding peak boundaries. 'creep' expands peaks based on a creeping window and makes wider peaks with the creep_dist option. 'bool' makes more concise peaks based on the pad distance. 'hyst' uses a hysteresis method to find edges of regions.")

# @optgroup.option('--hyst_threshold',
# 	default=0.5,
# 	type=float,
# 	help='Fraction of rpm_threshold that will be used for determining boundaries of a region. To turn of hysteresis, set this value to 1 (the method will work with this higher threshold).')
...
# @optgroup.option("--min_region_depth",
# 	default=30,
# 	type=int,
# 	help="Minimum depth in read abundance for a sRNA producing region to be called.")

# @optgroup.option("--min_region_resolution",
# 	default=0.15,
# 	type=float,
# 	help="Minimum depth in read abundance by genomic length for a sRNA producing region to be called. This is shown as a float of reads per nucleotide. I.e. 0.125 is 1 read / 8 nucleotides.")

# @optgroup.option("--min_locus_resolution",
# 	default=0.15,
# 	type=float,
# 	help="Like min_region_resolution, but applied to total loci after calling.")
...
def decompose(**params):
	...
	# params['output_directory'] = str(output_directory)
	# params['alignment_file'] = str(alignment_file)
	# params['project_name'] = project_name
	pprint(params)

	if params['name']:
		dir_name = f"decompose_{params['name']}"
	else:
		dir_name = 'decompose'
	...
	# bw = pyBigWig.open('test.bigwig', 'w')
	# bw.addHeader(chromosomes)
	# # bw.addHeader([("chr1", 1000000), ("chr2", 1500000)], maxZooms=0)
	# bw.addEntries(["NC_003070.9", "NC_003070.9", "NC_003070.9"], [0,100, 125], ends=[5,125, 126], values=[0, 1.0, 200.0], validate=False)
	# # bw.addEntries('NC_003070.9', starts=[1], values=[99], span=20)
	# bw.addEntries
	...
	# a_threshold = aligned_read_count / 1000000 * rpm_threshold

	# print(f" Read abundance thresholds:")
	# print(f"    minimum region detection .. {math.ceil(a_threshold)}  ({aligned_read_count} / 10^6 * {params['rpm_threshold']})")
	...
	peak_c, kernel_c = get_kernel_coverage(bam=alignment_file, 
		rgs=annotation_readgroups, 
		params=params, chrom_depth_c=chrom_depth_c, chromosomes=chromosomes)
	...
	counters_to_bigwig(peak_c, chromosomes, Path(output_directory, dir_name, "coverage.bigwig"))
	counters_to_bigwig(kernel_c, chromosomes, Path(output_directory, dir_name, "kernel_mean.bigwig"))
	...
	sample_kernels = [kernel_c[c][p] for c,p in sample(positions, 10000)]
	ps = [0.1, 1, 5, 10, 25, 50, 75, 90, 95, 99, 99.9]
	qs = np.percentile(sample_kernels, ps)

	for i,q in enumerate(qs):
		print(ps[i], q, sep='\t')

	print(np.percentile(sample_kernels, 90))
	q = np.percentile(sample_kernels, 90)
	trim_c = {}
	for chrom in kernel_c.keys():
		trim_c[chrom] = Counter()
		for p in kernel_c[chrom].keys():
			depth = kernel_c[chrom][p]

			if depth > q:
				trim_c[chrom][p] = depth
	counters_to_bigwig(trim_c, chromosomes, Path(output_directory, dir_name, "trim_mean_p90.bigwig"))


	print(np.percentile(sample_kernels, 95))
	q = np.percentile(sample_kernels, 95)
	trim_c = {}
	for chrom in kernel_c.keys():
		trim_c[chrom] = Counter()
		for p in kernel_c[chrom].keys():
	...
	counters_to_bigwig(trim_c, chromosomes, Path(output_directory, dir_name, "trim_mean_p95.bigwig"))
	...
	for chrom_count, chrom_and_length in enumerate(chromosomes):
		...
		# min_region_depth_fail = 0
		# min_region_resolution_fail = 0
		...
		candidate_peak_count = len([v for v in kernel_c[chrom].values() if v / aligned_read_count * 1000000 > rpm_threshold])
		...
		def get_kernel_boundaries(kernel_c, cursor, p_threshold, claim_d):
			...
			for direction in [-1,1]:
				...
				while True:
					c += direction
					depth = kernel_c[c]
					if depth < p_threshold or c in claim_d:
						boundaries.append(c + direction)
						break
		...
		for center, depth in kernel_c[chrom].most_common():
			...
			if center not in claim_d:
				...
				depth_threshold = math.ceil(depth * peak_threshold)
				...
				if rpm < rpm_threshold:
					...
					# 	lbound  = creep(-1, center, depth_threshold, peak_c, claim_d, chrom, creep_dist, pad)
					# 	rbound  = creep( 1, center, depth_threshold, peak_c, claim_d, chrom, creep_dist, pad)

					# elif boundary_method == 'bool':
					# 	lbound  = boolean_creep(-1, center, depth_threshold, peak_c, claim_d, chrom)
					# 	rbound  = boolean_creep( 1, center, depth_threshold, peak_c, claim_d, chrom)

					# elif boundary_method == 'hyst':
					# 	lbound, rbound = hysteresis(center, params, peak_c[chrom], claim_d, h_threshold)

					# print(rpm, depth, depth_threshold)
					lbound, rbound = get_kernel_boundaries(kernel_c[chrom], center, depth_threshold, claim_d)
					lbound, rbound = resolve_overlaps(lbound, center, rbound)


					total_region_space += rbound
					total_region_space -= lbound


					claim = f"region_{cl_i}"

					for r in range(lbound, rbound + 1):
						claim_d[r] = claim

					# region_depth = max([peak_c[chrom][r] for r in range(lbound, rbound+1)])
					# region_resolution = region_depth / total_region_space

					# if region_depth < params['min_region_depth']:
					# 	# print('region failed min depth')
					# 	min_region_depth_fail += 1
					# elif region_resolution < params['min_region_resolution']:
					# 	# print('region failed min resolution')
					# 	min_region_resolution_fail += 1
					# else:
		...
		# print()
		# print(min_region_resolution_fail, 'region failed min resolution')
		# print(min_region_depth_fail, 'region failed min depth')
		# print()
		...
		def final_check_and_increment(i):
			locus_length = loci[i][3] - loci[i][2]
			locus_depth  = sum(strand_d[loci[i][0]].values())
	...
	print(f"{total_region_space:,} total region space (bp)")
	print()
	print()
