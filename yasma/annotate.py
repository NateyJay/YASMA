...
# x = [1,5,6,3,12,345,43,56,7,34,5,7,33,54,3,2,3,45,37,78,8,99]

# d = deque([0,0,0])
# rolling_mean = 0

# for xx in x:
# 	print(xx, mean(d), rolling_mean, sep='\t')

# 	rolling_mean -= d.popleft()/3
# 	rolling_mean += xx / 3
# 	d.append(xx)
...
def hysteresis(cursor, params, peak_c, claim_d, h_threshold):

	# h_threshold    = math.ceil(d_threshold * params['hyst_threshold'])
	p_threshold    = peak_c[cursor] * params['peak_threshold']

	# print('d_threshold:', d_threshold)
	# print('h_threshold:', h_threshold)

	if cursor in claim_d:
		sys.exit("peak within annotated locus")

	boundaries = [cursor, cursor]
	# print()
	...
	for i,direction in enumerate([-1, 1]):
		...
		while True:
			...
			if depth == 0:
				...
				# print('claimed')
				break
			elif depth < h_threshold:
				# print(f'h_threshold -> {depth} < {h_threshold} < {d_threshold}')
				break
			elif depth < p_threshold:
				# print(f'p_threshold -> {depth} < {p_threshold}')
				break

		boundaries[i] = c - direction

	for i,direction in enumerate([-1, 1]):

		c = boundaries[i]
		for r in range(params['pad']):
...
def boolean_creep(direction, cursor, depth_threshold, peak_c, claim_d, chrom):
	"""
...
def creep(direction, cursor, depth_threshold, peak_c, claim_d, chrom, creep_dist, pad):
	"""
	Defines peak boundaries, based on a minimum threshold in a given direction.
...
def annotate(**params):
	...
	if annotation_name:
		dir_name = f"annotate_{annotation_name}"
	else:
		dir_name = 'annotate'
