...
class locusClass():
	...
	def __init__(self, threshold, merge_dist, chrom_length, chrom, alignment_file, rgs, total_reads, rpm_threshold):
		self.in_locus     = False
...
def annotate(alignment_file, annotation_readgroups, gene_annotation, output_directory, force, sensitivity, window, merge_dist, pad, peak_trim):
	...
	for chrom_count, chrom_and_length in enumerate(chromosomes):
		...
		for t in range(max_t):
			loci_d[t] = locusClass(t, merge_dist, chrom_length, chrom, alignment_file, annotation_readgroups, total_reads, rpm_threshold)
		...
		for t in range(max_t):
			...
			# print(r, depth_c[r], sep='\t')
				loci_d[t].add(depth=depth_c[r], position=r)


			for i, locus in enumerate(loci_d[t].loci):
				# print(i, locus)
				...
				for r in range(start, stop+1):
					try:
						names.update(read_position_d[r])
					except KeyError:
						pass
				...
				rpm = len(names) / total_reads * 1000000

				length = stop - start
				gap = start - last_stop


				loci_d[t].loci[i].append(length)
				loci_d[t].loci[i].append(gap)
				loci_d[t].loci[i].append(rpm)

				last_stop = stop


			loci_d[t].loci = [l for l in loci_d[t].loci if l[-1] >= rpm_threshold]
		...
		for t in range(max_t):
			...
			med_length  = median([l[-3] for l in loci_d[t].loci])
			med_gap     = median([l[-2] for l in loci_d[t].loci])
			mean_length = mean([l[-3] for l in loci_d[t].loci])
			mean_gap    = mean([l[-2] for l in loci_d[t].loci])


			with open(assess_file, 'a') as outf:

				print(chrom, t, len(loci_d[t].loci), med_length, mean_length, med_gap, mean_gap, sep='\t', file=outf)
