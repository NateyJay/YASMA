...
def coverage(alignment_file, annotation_readgroups, dicercall, output_directory, force, partial_wigs):
	"""Produces wig and bigwig coverage files for use in jbrowse."""
		...
		while sam_chrom == chrom:
			...
			if pos == sam_pos:
				...
				for size in coverage_names:
					...
					wig_d[size].add(round(cov * read_equivalent,4), pos, chrom)
