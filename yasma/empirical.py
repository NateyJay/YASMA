...
def annotate(alignment_file, annotation_readgroups, gene_annotation, output_directory, force, window, merge_dist, pad, peak_trim):
	...
	print("calculating empirical thresholds...")

	def calc_thresholds():

		threshold_d = {}
		file_name = f'{output_directory}/Empirical/Thresholds.txt'
		...
		if isfile(file_name):
			...
			log_file = f'{output_directory}/Empirical/Thresholds_log.txt'


			with open(log_file, 'w') as outf:
				print('chrom','depth','freq','p','cumulative_p', sep='\t', file=outf)
			...
			for c,l in chromosomes:
				...
				# print(c)
				depths = get_window_depths(alignment_file, c, window)

				depths = sample(depths, round(len(depths)/10))
				...
				for i in range(max(depths)):
					...
					with open(log_file, 'a') as outf:
						print(c, i, depths[i], round(p,6), round(cumulative_p,6), sep='\t', file=outf)

					running.popleft()
					running.append(p)

					if sum(running) / len(running) < 0.001:
						break

					threshold_d[c] = i

			with open(file_name, 'w') as outf:
				for c,i in threshold_d.items():
					print(c,i, sep='\t', file=outf)

		return(threshold_d)

	threshold_d = calc_thresholds()
	...
	for chrom_count, chrom in enumerate(chromosomes):
		...
		threshold = threshold_d[chrom]

		output_d['threshold'][chrom] = threshold
